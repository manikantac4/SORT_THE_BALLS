...
function Ball3D({ position, targetPosition, color, isAnimating }) {
  ...
  useFrame(() => {
    ...
    posRef.current[0] = THREE.MathUtils.lerp(posRef.current[0], targetPosition[0], lerpFactor)
    posRef.current[1] = THREE.MathUtils.lerp(posRef.current[1], targetPosition[1], lerpFactor)
    posRef.current[2] = THREE.MathUtils.lerp(posRef.current[2], targetPosition[2], lerpFactor)
...
function TimeUpOverlay({ isVisible }) {
  return (
    ...
    >
      {/* Soft ambient glow */}
      <div style={{
        position: "absolute", width: "clamp(300px, 560px, 80vw)", height: "clamp(300px, 560px, 80vw)", borderRadius: "50%",
      ...
      {/* Ping rings */}
      <div style={{
        position: "absolute", width: "clamp(250px, 440px, 70vw)", height: "clamp(250px, 440px, 70vw)", borderRadius: "50%",
        border: "2px solid rgba(180,60,255,0.28)", pointerEvents: "none",
        animation: isVisible ? "ping 2s cubic-bezier(0,0,0.2,1) infinite" : "none",
      }} />
      <div style={{
        position: "absolute", width: "clamp(180px, 310px, 60vw)", height: "clamp(180px, 310px, 60vw)", borderRadius: "50%",
      ...
      {/* Progress bar */}
      <div className="z-10 rounded-full overflow-hidden mt-12"
        style={{ width: "clamp(200px, 260px, 70vw)", height: 3, background: "rgba(255,255,255,0.08)" }}
      >
        <div
